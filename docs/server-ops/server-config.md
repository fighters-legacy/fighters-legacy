...
| `--campaign <file>` | campaign YAML | Run a **dynamic campaign** (#584): fly the campaign engine's current sortie — a story mission at its trigger, or a generated dynamic template with its `${...}` fills resolved. On the objective outcome the campaign advances and its state is saved to the [persistence store](#persistence--the-durable-store-533), so a restart continues the persistent war. A war saved before that store existed (`cache/campaign_<name>.flsave`) is imported on the next start, and the file is left in place; with `[persistence] enabled = false` the file stays the record. Composes with `--mission-report` for headless campaign testing. Campaign schema: [docs/modding/formats.md](../modding/formats.md). |
| `--time-rate <name>` | `paused` \| `eighth` \| `quarter` \| `half` \| `normal` \| `double` \| `quad` \| `octa` | Sim **wall-clock** rate applied after startup (#915). The sim step stays 1/60 s — content is byte-identical; ticks just arrive slower or faster in real time. Used by the cinematic demo recorder (`docs/developer/demo-recording.md`): a slow software-rendered client is served at `quarter`/`eighth` so it never misses a capture boundary. An unknown name logs a warning and uses `normal`. |
...
registration note in [docs/developer/architecture.md](../developer/architecture.md).
...
128+ multiplayer re-target so the [bot_swarm load harness](../developer/load-testing.md) can drive the server
...
field in each mod's `manifest.toml`. See [docs/developer/architecture.md](../developer/architecture.md) for the mod
manifest format.
...
> periodic cross-peer full-resync spike. See [network-protocol.md](../developer/network-protocol.md) → *Scaling to 128+*.
...
re-rolling the die (see the sensor decision record in [architecture.md](../developer/architecture.md)), so the
...
[congestion-control-design.md](../developer/decisions/congestion-control-design.md).
...
cannot meet the 2 s radio-comms budget; see `docs/developer/ai-architecture.md` §9).
...
Selects the network transport. See [transport-selection.md](../developer/decisions/transport-selection.md) and
[gns-backend.md](../developer/gns-backend.md).
...
[load-testing.md](../developer/load-testing.md)).
...
`127.0.0.1` (see [bot_swarm](../developer/load-testing.md)).
...
documented in [docs/developer/load-testing.md](../developer/load-testing.md#authoritative-server-tick-budget-server_tick).
...
[docs/developer/load-testing.md](../developer/load-testing.md#trace-replay-560).
...
specified in [replay-format.md](../developer/replay-format.md); it is versioned for real, because a replay
...
- **`[ai.provider]` / `[ai.mcp]`** *(Epics M–P — Dynamic World & Agentic AI, decision record
  ...
  [docs/developer/ai-architecture.md](../developer/ai-architecture.md).

Clustered deployments configure these through the `fl-operator` CRDs rather than hand-edited
TOML; see [docs/developer/decisions/distribution.md](../developer/decisions/distribution.md#server-distribution--self-hosting).

## See also

- [docs/developer/network-protocol.md](../developer/network-protocol.md) — wire format specification for all
