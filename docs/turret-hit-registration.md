...
Quantization follows `docs/snapshot-quantization.md`: az/el are bounded angles, a 12–14 bit
...
`docs/network-protocol.md`); the turret inherits it rather than inventing a new policy.
...
- **Additive `SnapshotCrewTurret` TLV** (`0x0106`, own-turret-only, quantized az/el per
  `docs/snapshot-quantization.md`); documented in `docs/network-protocol.md`. No `kProtocolVersion`
  bump.
...
- **Docs + CHANGELOG**: this document's status, `docs/network-protocol.md` TLV table, an
  `[Unreleased]` entry.
