...
| `Chat` | `0x20` | client→server | reliable | 4 + text | In-match chat line (#646, client half pending). Additive ID. |
| `ChatEvent` | `0x21` | server→client | reliable | 8 + text | Routed chat line (#646, pending). Additive ID. |
...
| `ServerQuery` / `ServerInfo` | `0x41` / `0x42` | client↔server | raw UDP | — | A2S-style server info query (#997, pending). |
