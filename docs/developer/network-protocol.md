...
| `SnapshotArticulation` | `0x0107` | records | `MsgWorldSnapshot` | **Articulation channel values** (#843, #1195) for the articulated entities in the receiving peer's interest set, so a remote aircraft's gear and flaps move and a swing-wing's wings sweep. Payload: repeated `{ uint32 entityIdx (LE), uint16 channelMask (LE), uint8 value[popcount(mask)] }`. Bit *n* of the mask is `ArtChannel` *n* (`engine/render/ArtChannel.h`; the enum order is the wire order and is append-only). Unsigned channels encode `round(v × 255)`, signed ones `round(v × 127) + 128` (offset binary). **Quantization is cosmetic-grade on purpose:** 1/255 on a 0..1 actuator is 0.4% — visually exact, and it cannot affect flight because *a peer never reads its own entity's channels from the wire* (client prediction overwrites them with its own full-precision integration). Sent on **change**, plus a periodic refresh every `kArtRefreshTicks` (30) for drop tolerance and interest re-entry. An entity at all-default channels is **absent**, so a world of unarticulated aircraft emits no TLV and its snapshot is byte-identical to pre-#843. Absence means HOLD-LAST, not neutral. The sender skips a channel sitting exactly on `artChannelNeutral` — **not** on `> 0`, which would drop every negative value on a signed channel. Which channels are populated is the flight model's business, not the wire's: `engine/flight/ArticulationChannels.h` is the single mapping the server and the client's own-aircraft path both build from, and a channel with no writer there is simply never sent. Wing sweep (`ArtChannel::Sweep`, normalized `(current - min_deg) / (max_deg - min_deg)`) joined it in #1195; a fixed-geometry aircraft has no `[wing_sweep]`, so its sweep is neutral, absent, and costs nothing. Little-endian, unaligned; read per-field via memcpy. |
