...
Algorithms the Vulkan renderer implements (see [`docs/developer/rendering.md`](rendering.md) for the pass-by-pass
...
| OpenSSL | Crypto backend for the GNS transport (the #506 spike's libsodium pick was reversed during #507 — GNS rejects libsodium AES on ARM/Apple Silicon; see `docs/developer/gns-backend.md`) | Apache-2.0 | [openssl.org/docs](https://www.openssl.org/docs/) |
...
behaviour; see [docs/developer/ai-architecture.md](ai-architecture.md).
...
For landscape context and motivation, see [`docs/developer/prior-art.md`](prior-art.md).
