...
| `IAsyncFilesystemHandler` | `platform/IAsyncFilesystem.h` | Callback target for async read completions; passed **per read** to `readFileAsync` (#1083) |
| `IAsyncFilesystem` | `platform/IAsyncFilesystem.h` | Non-blocking whole-file reads for per-frame terrain chunk streaming; completions dispatched via `service()`, routed by read id |
| `ILogger` | `platform/ILogger.h` | Structured logging routed to the platform-native output |

**Event handler pattern:** `IWindowEventHandler`, `INetworkEventHandler`, `ITextInputHandler`, `IAsyncFilesystemHandler`, `IHttpClientHandler` and `IJoystickEventHandler` are separate interfaces. The engine implements the handler; the platform backend calls it during `pollEvents()` / `service()` / text input events / async I/O completions / hot-plug events. This keeps platform-to-engine callbacks decoupled without requiring the backend to know anything about the game loop.

**Registration is per REQUEST where a HAL is request/response** — `IAsyncFilesystem::readFileAsync` and `IHttpClient::request`/`get` take the handler as an argument, and completions route by the id they already return (#1083). Both used to have a single-slot `setEventHandler`, and that is a silent-steal bug waiting for a second consumer: `ContentDownloader::start` took the HTTP slot and `finish()` set it back to **`nullptr`** rather than restoring what was there, while the client already owned that slot for the server browser's lobby list. Wiring the join-time download (#924) or first-run pack download (#108) would have ended lobby callbacks for the rest of the process — no error, no log, a browser that never populates again. A handler **list** was considered and rejected: every subscriber would then filter every completion, turning a silent steal into silent crosstalk.

A departing consumer calls `cancelReadsFor(this)` / `cancelRequestsFor(this)`, which cancels its in-flight work **and suppresses those completions**. That is what replaces `setEventHandler(nullptr)` in a destructor: plain `cancelRead`/`cancel` still deliver a terminal callback, straight into the object being destroyed — `TerrainStreamer`'s destructor deregistered first for exactly that reason. A null handler is refused at submission (`readFileAsync`/`request` return 0) rather than accepted and dropped.
...
- **Game loop** (`engine/loop/`) — `TimeController` (fixed-timestep accumulator, time compression), `GameLoop` (sim thread, frame gating, render alpha). `ISimUpdate` is the callback interface for game systems advancing each tick, and `GameLoop` drives an **ordered list** of them: registration order is execution order, so tick order is data the loop holds (#1078). It took exactly one system before, which is why fl-server hand-sequenced five sim systems inside a single `setMissionTickHook` lambda across three different step signatures — and why `AlertSystem` implemented `ISimUpdate` while carrying a comment explaining that it was *not* registered with the loop it implemented the interface for. A system's sub-rate cadence (1 Hz bot backfill, a 30-tick world-state push) is now its own concern rather than the caller's. `GameLoop::stepOnce()` runs that same list on the calling thread for a harness that owns its tick loop (the deterministic `--mission-report` run), so there is one definition of tick order rather than a second copy in the harness. `GameLoop::enqueueSimCallback()` allows any thread to queue a one-shot lambda that runs at the top of the next sim tick — used by the game console to dispatch entity mutations safely.
...
**Coordinate convention:** the planet centre sits at `{0, -R, 0}` in world space, where R = `planet_radius_m`, and world **+Y is the polar axis** — so the world origin (`{0, 0, 0}`) is the **north pole** (lat 90°, longitude undefined), not lat 0/lon 0. Content is therefore not authored there: the sandbox home (`engine/world/SandboxHome.h`) is 36.24917° N, 114.99611° W at ~570 m, where the builtin airfield stands, a mission's `anchor:` measures from, and a server with no configured spawn points puts a pilot (#1211).
