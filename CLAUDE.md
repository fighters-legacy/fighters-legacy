...
**Wire protocol:** fully documented in `docs/network-protocol.md`. `kProtocolVersion = 1`; `MsgHello` is 4 bytes (first reliable packet on connect; client disconnects on version mismatch); `MsgEntityEntry::pos[3]` is `double` (68 bytes); `MsgConnectAck` is 12 bytes (includes `assignedEntityIdx/Gen`); `MsgClientInput` is 44 bytes (client→server, reliable). Always parse with `std::memcpy`. #92 (version negotiation), #142 (authoritative game protocol), and #170 (double-precision positions) are all merged.
...
- `engine/net/GameProtocol.h` — six packed wire types; `kProtocolVersion = 1`; `MsgHello` 4 bytes (`MsgId::Hello = 0x00`); `MsgConnectAck` 12 bytes (+assignedEntityIdx/Gen), `MsgEntityEntry` 68 bytes (double pos), `MsgClientInput` 44 bytes (`MsgId::ClientInput = 0x03`); `MsgWorldSnapshotHeader` offset 1 = `protocolVersion uint8_t`; `MsgClientInput` offsets 2–3 = `protocolVersion uint16_t`; always parse with `memcpy`; all documented field offsets protected by `offsetof` static_asserts (requires `<cstddef>`); see `docs/network-protocol.md`
- `docs/network-protocol.md` — stable public wire protocol spec: channel assignments, struct field tables (Offset/Size/Field/Type/Notes), little-endian declaration, ARM64 alignment warnings for misaligned fields (`pos[3]` at offset 12, `tickIndex` at offset 4 in snapshot header), version negotiation implemented (#92) via `MsgHello` handshake + per-packet `protocolVersion` echo, bandwidth/MTU scalability guidance (single-fragment threshold ≈20 entities; interest management required for 128+ players)
