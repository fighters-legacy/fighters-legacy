...
- **`engine/net/GameProtocol.h`**: packed wire types (`#pragma pack(1)`). Always parse with `std::memcpy` — never cast raw packet pointers. `kProtocolVersion = 1` (uint16_t). Eight message types: `MsgHello` (4 bytes, `MsgId::Hello = 0x00`, server→client version handshake), `MsgConnectAck` (12 bytes, includes `assignedEntityIdx/Gen`), `MsgEntityTypeDef` (196 bytes), `MsgWorldSnapshotHeader` (12 bytes, offset 1 = `protocolVersion uint8_t`), `MsgEntityEntry` (68 bytes, double pos), `MsgClientInput` (44 bytes, `MsgId::ClientInput = 0x03`, offsets 2–3 = `protocolVersion uint16_t`), `MsgWeatherState` (20 bytes, `MsgId::WeatherState = 0x04`, server→client unreliable ~6 Hz; carries `preset uint8_t`, `timeOfDayTenths uint16_t` [hours×10], `fogDensity`, `fogStartDist`, `windX`, `windZ` — additive ID, old clients silently discard, no version bump), `MsgLanBeacon` (74 bytes, `MsgId::LanBeacon = 0x10`, raw UDP broadcast not sent over ENet). `kGameModeCampaign/Mission/Sandbox` bit constants for `MsgLanBeacon::gameModeFlags`. Wire quaternion order: `x, y, z, w` matching `EntityTransform::quat`. See `docs/network-protocol.md` for field tables.
- **`engine/net/WorldBroadcaster`**: `ISimUpdate` + `INetworkEventHandler`. `onConnect` check order: (1) ban check (`m_bannedAddresses` — disconnect + return if banned), (2) allowlist check (`m_allowedAddresses` — if non-empty and IP not listed, disconnect + return), (3) rate-limit check (sliding window per IP using `m_connectRecords` — if size > `m_connectRateLimit`, disconnect + return); then sends `MsgHello` (reliable), spawns `"builtin:debug-entity"` for the peer, creates a `FlightIntegrator` stored in `m_peerFlightSims`, and sends `MsgConnectAck` + `MsgEntityTypeDef[]`. `onDisconnect` kills the entity and erases peer from all maps including `m_peerFloodState`. `onReceive` checks `MsgClientInput` flood rate in a 1-second window (`m_peerFloodState`; threshold = `60 * m_floodMultiplier`), then validates `protocolVersion` and stores → `PeerInputState`. `onTick` steps each peer's `FlightIntegrator`, calls `EntityManager::onTick`, builds and broadcasts `MsgWorldSnapshot`, broadcasts `MsgWeatherState` every 10 ticks (~6 Hz), and prunes stale `m_connectRecords` entries every 600 ticks. Constructor takes optional `WeatherController* weather = nullptr`. `engine-net` privately links `engine-flight` and `engine-weather`. `getPeerCount() const noexcept` — reads `std::atomic<int> m_activePeerCount`; safe to call from any thread. **Peer management (sim-thread only):** `kickPeer(peerId)` — calls `INetwork::disconnectPeer`; `banAddress(ip)` — normalises IP, inserts into `m_bannedAddresses`, kicks matching peers; `unbanAddress(ip)` — erases from ban set; `forEachPeer(fn)` — iterates `m_peerEntities`, copies each `getPeerAddress` string per entry; `setBannedAddresses(set)` — replaces entire ban set (used at startup to load banlist file); `setAllowedAddresses(set)` — replaces allowlist (empty = disabled); `getBannedAddresses()` — returns copy of current ban set; `setRateLimitParams(maxConnects, windowSeconds, floodMultiplier)` — configure rate limiting before `gameLoop.start()`; `setClockOverride(fn)` — inject a clock function (tests only).
- **`ClientNetEventHandler`** (in `game/fighters-legacy/main.cpp`): parses `MsgHello` (checks `protocolVersion == kProtocolVersion`, calls `net.disconnect()` on mismatch), `ConnectAck` (stores `assignedEntityIdx/Gen`), `WorldSnapshot` (calls `bridge.publishExternal()`). Sends `MsgClientInput` (carries `protocolVersion`) to the embedded server each frame (arrow keys + shift + space).
...
**Wire protocol:** fully documented in `docs/network-protocol.md`. `kProtocolVersion = 1`; `MsgHello` is 4 bytes (first reliable packet on connect; client disconnects on version mismatch); `MsgEntityEntry::pos[3]` is `double` (68 bytes); `MsgConnectAck` is 12 bytes (includes `assignedEntityIdx/Gen`); `MsgClientInput` is 44 bytes (client→server, reliable); `MsgWeatherState` is 20 bytes (`MsgId::WeatherState=0x04`, unreliable ~6 Hz, additive ID); `MsgLanBeacon` is 74 bytes (`MsgId::LanBeacon=0x10`, raw UDP broadcast, not ENet — received by `DiscoveryListener`). Always parse with `std::memcpy`. #92 (version negotiation), #142 (authoritative game protocol), #170 (double-precision positions), #91 (LAN discovery), and #39 (weather/time-of-day) are all merged.
...
- `engine/debug/DebugConsole.h` — Quake-style drop-down HUD overlay; backtick toggle (SDL_SCANCODE_GRAVE in main.cpp); `ITextInputHandler` impl; output ring (64 entries); position widget (`toggle_pos`); `buildHud(playerPos*)` / `elements()` / `showPosRef()` / `openHeadless()` (test helper); no SDL dep (uses IInput for special keys)
...
- `engine/net/GameProtocol.h` — eight packed wire types; `kProtocolVersion = 1`; `MsgHello` 4 bytes (`MsgId::Hello = 0x00`); `MsgConnectAck` 12 bytes (+assignedEntityIdx/Gen), `MsgEntityEntry` 68 bytes (double pos; offset 66 = `throttle` uint8_t [0–100]; offset 67 = `fuelPct` uint8_t [0–100]), `MsgClientInput` 44 bytes (`MsgId::ClientInput = 0x03`); `MsgWorldSnapshotHeader` offset 1 = `protocolVersion uint8_t`; `MsgClientInput` offsets 2–3 = `protocolVersion uint16_t`; `MsgWeatherState` 20 bytes (`MsgId::WeatherState = 0x04`, unreliable ~6 Hz, `timeOfDayTenths uint16_t` at offset 2 [hours×10]); `MsgLanBeacon` 74 bytes (`MsgId::LanBeacon = 0x10`, raw UDP only); `kGameModeCampaign/Mission/Sandbox` bitmask constants; always parse with `memcpy`; all documented field offsets protected by `offsetof` static_asserts (requires `<cstddef>`); see `docs/network-protocol.md`
- `engine/net/WorldBroadcaster.h` — `ISimUpdate` + `INetworkEventHandler`; constructor takes optional `WeatherController* weather = nullptr`; `onConnect` check order: ban → allowlist → rate-limit (sliding window); `onReceive` flood detection (1-second packet-rate window per peer); `onTick` prunes stale rate-limit entries every 600 ticks; `getPeerCount() const noexcept` atomic cross-thread read; **sim-thread peer management:** `kickPeer(peerId)`, `banAddress(ip)`, `unbanAddress(ip)`, `forEachPeer(fn)`, `setBannedAddresses(set)`, `setAllowedAddresses(set)`, `getBannedAddresses()`, `setRateLimitParams(maxConnects, windowS, floodMul)`, `setClockOverride(fn)` (tests); private: `m_bannedAddresses`, `m_allowedAddresses`, `m_connectRecords` (per-IP deque of timestamps), `m_peerFloodState` (per-peer packet count + window start), `m_now` (injectable clock)
