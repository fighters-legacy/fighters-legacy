...
- **`engine/net/WorldBroadcaster`**: `ISimUpdate` + `INetworkEventHandler`. `onConnect` spawns `"builtin:debug-entity"` for the peer, creates a `FlightIntegrator` (via `BuiltinFlightModel::get()`) stored in `m_peerFlightSims`, and stores the entity in `m_peerEntities`; `onDisconnect` kills the entity and erases both maps. `onReceive` validates and stores `MsgClientInput` → `PeerInputState` (`m_peerInputs`). `onTick` steps each peer's `FlightIntegrator` from stored inputs (maps `PeerInputState` → `ControlInput`), copies `FlightState` → `EntityState`, then calls `EntityManager::onTick` and serialises all live entities into `MsgWorldSnapshot` and broadcasts unreliably. Sends `MsgConnectAck` (with assigned entity slot) + `MsgEntityTypeDef[]` on connect (reliable). `engine-net` privately links `engine-flight`.
