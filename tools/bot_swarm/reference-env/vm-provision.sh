...
#
# openssl-devel + protobuf-devel/-compiler are the GameNetworkingSockets deps (#649): the `gns`
# scale-gate profile builds with FL_ENABLE_GNS=ON so the DEFAULT internet transport is validated at
# 128 clients, not just enet6. GNS v1.6.0 needs the PRE-ABSEIL protobuf 3.21.x line, which Fedora
# still ships (3.19.x) — see docs/gns-backend.md. Without these, cmake/dependencies.cmake silently
# force-disables GNS and the leg would measure enet6; the workflow asserts FL_ENABLE_GNS stayed ON
# after configure so that can't pass silently.
dnf -y install \
