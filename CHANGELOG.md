...
- **game**: Chase view (F2) is now locked to the aircraft's tail — the orbit yaw tracks the entity heading (`atan2(fwd.x, fwd.z)`) so the camera trails directly behind as the aircraft turns, rather than holding a fixed world-space angle (which left it in front of the aircraft). The chase pitch was also lowered from 10° to 3° so the camera sits nearly level behind the tail instead of looking down on it. LMB drag still orbits relative to "directly behind".
- **game**: The builtin placeholder entity now rests ON the terrain instead of half-buried. Its tetrahedron mesh is centred on the entity origin, so fl-server lifts the spawn elevation and physics floor by the mesh's centroid-to-base distance (`kBuiltinEntityGroundClearanceM`, ~9.43 m) — a sandbox placeholder until content meshes carry per-mesh ground-contact bounds.
