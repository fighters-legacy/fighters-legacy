...
- **game**: Single-player mission lifecycle — flying a selected mission now actually loads it (#634). Picking a mission and hitting Fly forwards the choice through `Game::startGame` to the embedded `LocalServer`, which passes `--mission <name>` to the fl-server it spawns (empty = the sandbox, exactly as before), so the world comes up with the mission's objects, factions, and weather instead of the empty sandbox. The mission-brief screen renders the mission's real display name, loaded through the same engine-mission parser the server uses (the game binary now links `engine-mission` — the client-side half of the #632 "runtime lives in the engine" decision), so the brief and the flown mission cannot disagree. The debrief now also accrues this session's flight time into `PilotProfile::flightTimeS` alongside the real kills/losses it already recorded. A mission object with no explicit `ai:`/`route:` now falls back to its entity type's own default AI script (the same auto-detect the `spawn` admin command does), so a mission's stock aircraft fly themselves without every author restating `ai: lua <script>`. Building on the #876 crash fix (a session now starts on any entry into Loading).
- **tools**: Missions-as-integration-tests harness (#856) — the force multiplier for the rest of Phase 4. `fl-server --mission <name> --mission-report <path>` runs a mission **headless to completion** with no clients — a deterministic fixed-step loop (no wall-clock, no sim thread, the overrun governor pinned off so AI decimation can't make the outcome load-dependent) — and writes a JSON outcome (`outcome`/`elapsed_seconds`/`ticks`/`triggers_fired`/`live_entities`/`spawned_objects`, atomic via `writeConfigFile`, `ServerTickReport`-shaped) before exiting. A Python wrapper (`tools/mission_test/mission_test.py`, `scale_gate.py` precedent) launches the run and asserts on the report, wired into ctest as `mission_harness_ci_smoke` against a new `ci-mission-pack` fixture mission (deterministic, builtin entities only). This turns the mission format into the engine's integration-test surface: from here, weapons/vehicles/audio/avionics features can be verified by scripted missions in CI rather than manual flights. Verified end-to-end: the smoke mission runs to `success` in ~1 s of wall-clock.
- **mission**: Objective and trigger evaluator (#633). A new `MissionRuntime` (engine-mission) evaluates each trigger's `on` predicate server-side at a second-scale cadence (not every 60 Hz tick) and fires its `do` action exactly once, the first time it becomes true, in file order — `mission_start`, `timer(<seconds>)`, and `destroy(<id>)` predicates drive a single objective state machine (`Active → Complete/Failed`) via `mission_success`/`mission_failure`, while other actions route through an injected dispatcher (the validated-command seam). Evaluation is fully deterministic (fixed sim-dt tick clock, edge-once, no wall-clock or RNG) — the property the #856 headless harness needs to assert an outcome. It reaches the world only through `EntityManager` (liveness for `destroy`) and is driven by a new `WorldBroadcaster::setMissionTickHook` run at the end of `onTick` — GameLoop drives a single `ISimUpdate`, so the hook is the clean seam for a second sim-side consumer and keeps engine-net free of an engine-mission dependency. `applyMission` now returns the object-id→EntityId map the evaluator needs. fl-server builds the runtime when a mission loads and logs the outcome on completion.
- **mission**: Scripted-bot mission schema — per-object `ai`, `route`, and `loadout` (#855). A mission object can now be given a brain (`ai: "lua <script>"` for a pack Lua behavior, or a C++ behavior + args like `pursuit 1` / `patrol_attack 0` via the same grammar as `spawn --ai`), a `route:` of waypoints it flies (a waypoint controller; wins over `ai` when both are present), and a `loadout:` that overrides the entity's default payload station-by-station (a store must be in that station's `allowed` list; `~`/`-`/`""` empties a station; a partial list keeps the rest at their defaults). The engine-mission parser validates the SHAPE of these fields (shared with `validate-mission` — parity holds), and `applyMission` gains an `onSpawned` hook so fl-server — where engine-ai, engine-script, and the weapon registry live — turns them into controllers and loadouts, keeping engine-mission free of those dependencies. New `buildLoadoutOverride` (engine-weapon) and `WorldBroadcaster::setEntityLoadout` re-cost the airframe's payload from the actual mounted stores. Verified end-to-end: an fl-base-pack mission with `ai: lua fighter`, `ai: loiter …`, and a `loadout:` override loads cleanly.
- **server**: fl-server loads a mission at startup (#854). `--mission <name>` (or the first `[rotation]` item) resolves the mission asset, hands its bytes to the #632 runtime parser, and sets up the sim **before `gameLoop.start()`** — spawns and factions via `applyMission`, the coalition registry handed to the broadcaster. A malformed or missing mission logs and starts empty rather than aborting. `[rotation]` stops logging "not yet active" and loads its first item (multi-item rotation *timing* lands incrementally). Mission objects marked `player: true` become **joinable player slots**: a connecting pilot is now assigned an open slot — inheriting its faction, spawn position, and aircraft type (the slot overrides the client's requested type) — instead of the round-robin spawn-point + `[world] player_faction` path; the slot frees on disconnect and rebinds the next pilot, and a pilot beyond the available slots falls back to the default spawn so everyone still gets an aircraft. `WorldBroadcaster` gains `setFactionRegistry`/`setMissionPlayerSlots` and an extracted `spawnPilotEntity` core shared by both spawn paths; engine-net stays free of an engine-mission dependency (fl-server translates the runtime's player slots into the broadcaster's POD).
- **mission**: Mission YAML runtime parser and faction-aware sim setup (#632) — the root of the mission runtime (Epic #584). A new `engine-mission` library owns the mission schema (`docs/modding/missions.md`) as one parser (`parseMission` → a runtime `Mission` model + accumulated diagnostics) and `validate-mission` now **delegates** to it, so the linter a mod author runs and the engine that loads the mission can no longer drift (the same anti-drift rule as validate-weapon/-sensor/-entity). The runtime lives in the engine, not fl-server, because the game client needs it too (single-player lifecycle #634, Instant Action #40). `applyMission` sets up the sim from a parsed mission: it spawns each object at its position/heading with its faction, applies the mission's weather/time/wind, and returns joinable **player slots** (`player: true` objects, for the #854 handshake) rather than spawning them. The schema gains real **coalitions** — a side may declare `allies`, and the `FactionRegistry` relationship matrix (until now dead code) becomes the source of truth for AI friend-or-foe: `FactionRegistry::areHostile` + a nullable-registry `hostile()` seam thread through the AI conditions and threat-designation helpers via `AiTickContext::factions` and `WorldBroadcaster::setFactionRegistry`, with a null registry preserving the prior "distinct non-zero faction = hostile" behavior byte-for-byte (every existing AI/broadcast test unchanged). Index 0 is reserved neutral so a real side never lands on the unhostile index.
- **network**: Unified connect handshake (#853/#834/#857/#872). The client now speaks first at connect: it sends `MsgConnectRequest` — role (pilot/observer), requested entity type, and a mounted-pack manifest — and the server replies `MsgConnectAck` (with the **granted** role + assigned entity) or `MsgConnectRefusal`, replacing the old "server unilaterally spawns `builtin:debug-entity` on connect" flow. On top of that one seam: (1) **the player flies a configurable, client-selectable aircraft** — `[world] player_entity_type` sets the spawn default and a client `--aircraft <id>` requests a specific type, server-clamped to a registered type, so "boot a server, connect, look at the aeroplane" is a config change, not an engine patch (#834); (2) an **observer role** — a peer can join as a spectator with no entity/`FlightIntegrator`/controller and switch pilot⇄observer mid-session without reconnecting (`set_role <peerId> <pilot|observer>` admin command, `--observer` client flag, `[world] allow_observers`), the per-peer snapshot pass now delivering to entity-less observers (#857); (3) a **client pack manifest** on the wire with a warn-only required-pack policy (`[mods] required`) — a client missing a server-required pack is logged, not refused, instead of silently rendering placeholders (#872 wire half). `kProtocolVersion` stays 1 (primary dev). Full field tables in `docs/network-protocol.md`.
- **render**: mesh texture pipeline — meshes render with their authored textures instead of flat grey (#833). `createMesh` now consumes the primitive's PBR material: the engine layer supplies a URI→bytes resolver (`MeshUploadDesc::textureResolver`, wired to `AssetManager::loadTexture` because URI→asset-name→file mapping is a content-pack concern, not a GPU-backend one), the renderer uploads the base-color/normal/ORM textures and builds one GPU material, and `IRenderer::getMeshMaterial()` surfaces it so `SceneRenderer::getOrUploadMaterial` returns the mesh's real material (falling back to shaded grey only when none is authored). The texture URI convention is settled: a mesh references `../../textures/<name>.ktx2` (a bare `<name>.ktx2` is also accepted), resolving to the `textures/` asset directory; `validate-mesh` now accepts the spec-conformant `KHR_texture_basisu` extension (rejecting it made the correct authoring form fail CI) and warns on a texture URI whose extension is not `.ktx2`/`.png`; `docs/modding/textures.md` is corrected to the `textures/` layout. Multi-primitive/node-aware mesh loading stays with #839
- **game**: Weapon release/impact audio and haptics (#631): `SfxManager` plays positional one-shot weapon SFX (16-voice steal-oldest pool, full-decode buffer cache, camera-relative 3D positioning matching the renderer's invariant) from a preset vocabulary (`sfx.gunfire`/`launch`/`release`/`impact`/`explosion`) that a content pack overrides by asset name, with compiled-in procedural fallbacks (`SfxBuiltinSounds` — byte-stable deterministic PCM) so the guns are audible with zero content mounted; `ClientEffectRouter` gains the audio + haptic seam (own gunfire plays head-relative; an own-ship store release kicks the controller via `notifyOrdnanceRelease`), and a missile-approach warning is deliberately withheld to #529 to avoid leaking an inbound launch through the controller (a wallhack). A null audio device is a clean no-op, so CI never needs one
- **entity**: Per-subsystem damage granularity (#675): an optional `[damage.subsystems]` table (fixed vocabulary — engine_left/right, controls, avionics, hydraulics, fuel — each an independent HP pool with a hit-bias weight) routes combat damage to a specific system by a deterministic weighted pick that a directed hit biases by where it struck (body-frame quadrant). When a pool is exhausted the effect maps onto existing machinery: **engine_left/right → asymmetric thrust** (one engine out halves thrust and yaws the nose toward the dead engine — the Phase-4 acceptance item, and `engineFailFlags` was parsed-and-ignored until now), controls/hydraulics → lost control authority, avionics → sensor suite stripped to the eyeball, fuel → a tank leak. Absent table = exactly the pre-#675 3-level model (regression-tested)
- **engine**: Entity-entity collision detection (#630): a real collision phase in the parallel tick — broadphase via the spatial index (inflated for post-integrate drift), sphere-sphere narrow phase against `collision_radius_m` (category defaults: air/player 8 m, ground/naval 15 m; projectiles excluded — they have their own fuze path), detect data-parallel and serial-equivalent across worker counts, damage applied serially and scaled by relative speed to BOTH entities as environmental (a mid-air is nobody's kill and physics, not friendly fire), gated by the `crash_damage` difficulty toggle. The `collision_ms` profiler phase now measures actual collision work — it was a misnomer timing entity-manager housekeeping, which moves to `maintenance_ms`; the frozen tick-report schema (6) is untouched
- **ai**: Ballistic missiles fly themselves (#355): `BallisticGuidanceController` steers the boost phase to a commanded impact point — local-tangent bearing hold plus a lofted pitch program that ramps toward vertical as the continuously-predicted impact walks out to the target (with no thrust cutoff, horizontal velocity once banked is never given back, so range control must act early; a hard overshoot pushes past vertical to take speed back) — then flies inertial after burnout, like the real article; MIRV buses deploy child RVs past apogee through a new controller-spawn seam (`SpawnRequest` + `drainSpawnRequests`, drained serially in the weapons phase because controllers sample in a read-only parallel pass), children fan deterministically and inherit the launcher's ownership so an RV kill credits whoever fired the bus; factory grammar `--ai ballistic <tx> <ty> <tz> [mirvCount [spreadM]]`
- **flight**: Ballistic vehicles are real flight models (#354): `type = "ballistic"` selects a much smaller schema (`[engine.boost] thrust_n/burn_time_s`, no CL tables or turbine fuel flows — a booster has no wings and forcing authors to invent unread numbers helps nobody) flown by `BallisticForceModel` — constant boost thrust with thrust-vector control authority, inertial coast, and reentry deceleration emerging from the same drag term; `computeAtmosphere` extends past the old 20 km ceiling with the US Standard Atmosphere 1976 layers to 86 km and vacuum above, with the 0–20 km path kept BIT-IDENTICAL (regression-locked — fm-trim's CI gates and the generated manual quote those numbers); the integrator's Mach-6 NaN backstop is per-instance (`setSpeedGuard`) so an MRBM can legitimately outrun the guard built for aircraft
- **engine**: Bombs and unguided rockets fly real release ballistics (#629): a bomb is ejected DOWNWARD off the rack and falls on gravity + drag toward the AIR mass, so it drifts downwind by what the drag model says rather than a bolted-on offset; rockets get deterministic launch dispersion from the authored CEP (seeded per shooter/tick/round — a replayed salvo fans out identically) and RIPPLE while the release is held (a pod is a volume weapon); mass and drag shed per round so a half-empty pod costs half as much; `computeCcip` forward-integrates the same point-mass model, so the predicted impact and the actual bomb cannot disagree by more than integration phase
...
- **network**: The ENet `MsgId` space now reserves `0x00–0x1F` for ENet messages and `0x20+` for raw-UDP/non-ENet ids (`MsgLanBeacon` moved `0x10 → 0x20`), freeing an ENet id for `MsgConnectRequest` (`0x11`) — a deliberate, documented raise of the boundary the previous comment flagged. `MsgConnectAck` grew 16 → 20 bytes (appended `grantedRole` @16, still record-aligned) and three `ConnectRefusalCode` values were added (`RoleDenied`, `MissingRequiredPack`, `EntitlementRequired`). Both ends land together (primary dev), so no `kProtocolVersion` bump (#853)
...
- **weapon**: the loadout default-selection loops in `FireControl.cpp` used a `uint8_t` index compared against `stations.size()` (a `size_t`) — a `cpp/comparison-with-wider-type` defect that would loop forever on a >255-station airframe. The two `buildLoadout` loops and the `buildLoadoutOverride` helper now share one `pickDefaultSelection(size_t)` (bounded to `[0, 255]`, since `selected` is a `uint8_t`), removing the CodeQL alerts and the duplicated logic.
- **game**: flying a mission from the mission-select flow crashed the moment you clicked Fly — a null `LoadingScreen` dereference (a `_GLIBCXX_ASSERTIONS` abort in debug) (#876). `LoadingScreen`/`FlightScreen` are built lazily per session by `Game::startGame()`, but the trigger for that fired only on `MainMenu → Loading`; the mission path enters Loading from `MissionBrief`, so `startGame()` never ran and `ScreenManager::active()` dereferenced a null `m_loading`. The start/stop-session decision is now a pair of pure predicates (`entersSession`/`exitsSession` in `IScreen.h`): a session starts on **any** entry into Loading (`prev != Loading`), covering both the sandbox and mission paths, and the rule is unit-tested rather than an inline guard that drifted as new screens were added.
- **render**: a failed mesh load silently became the builtin placeholder tetrahedron and logged nothing — the empty-mesh-name case, an unloadable content pack, a wrong asset name, and a corrupt `.glb` all rendered identically with no diagnostic. `SceneRenderer` now warns once per cause: a mesh-less entity type (once per type index), a mesh name that resolves to no bytes in any pack, and a `.glb` that fails to upload (#832)
