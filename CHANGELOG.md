...
Entries are **generated from conventional-commit subjects** by [git-cliff](https://git-cliff.org)
when a release is cut (#1123) — do not edit this file by hand. Changes that are not yet in a
release are the commits on `main` since the
[latest release](https://github.com/fighters-legacy/fighters-legacy/releases/latest).

## [0.4.1] - 2026-09-17

### Added

- **server**: bans, accounts and stats survive a restart, in a store the server owns (#1394)
- **server**: accounts, bans and career stats have a schema, and a campaign lives in the store (#1395)
- **server**: a ban outlives the server, and says who issued it (#1396)

### Changed

- **roadmap**: close the Phase 4 gate — v0.4.0 shipped (#1387)
- **server**: an operator page for the persistence store (#1397)
- **roadmap**: phase 5 reads as executing, and three settled questions get their records (#1404)

### Fixed

- **tools**: keep the scale-gate report when a run fails its own asserts (#1388)
- **ci**: never auto-close a release task before its release ships (#1389)
- **engine**: size the sim worker pool from the CPUs the OS granted, not the host's cores (#1390)
- **content**: record the demo set 10 of 10, and make the checks that judge it mean something (#1392)
- **renderer**: light the sky from the LOCAL solar elevation, so an anchored mission has a night (#1393)
- **network**: GnsNetwork reports its bound port, and its suite stops naming ports (#1402)
- **netcode**: a spawn record names the participant who spawned, so a recording can carry pilot names (#1403)
- **net**: a registering server tells the lobby whether it is passworded and how often it heartbeats (#1405)
- **server**: a cleanly shut-down server is dropped from the lobby, and the DELETE carries its body (#1406)

## [0.4.0] - 2026-09-01

### Added

- **flight**: the builtin aircraft flies a realistic low-performance trainer instead of a UFO (#1341)
- **entity**: builtin coverage for every supported entity type (#1344)
- **ai**: the zero-pack sandbox gets an AI that can actually hit a SAM site (#1351)

### Fixed

- **ci**: scale-gate kbs tolerance gains an absolute floor for small baselines (#1343)
- **netcode**: snapshot fuel percent is a percent of the airframe's own capacity (#1346)
- **tools**: the load-test report says how many entities the run actually held (#1348)
- **ai**: a loitering aircraft flies the circle the mission asked for (#1349)
- **tools**: demo recordings capture the picture, run the whole shot list, and fail when they do not (#1350)
- **ai**: the AI hard deck is a height above the ground, not an altitude (#1356)
- **ai**: the AI flies a turn it can pay for, and unloads to escape a mush (#1357)
- **renderer**: light the terrain at midday — anchor-local mission clock, geometric terrain normals (#1361)
- **game**: a split HOTAS flies on the shipped defaults — every stick is read, and the keyboard throttle works beside a lever (#1363)
- **renderer**: record demos at 232 FPS instead of 2.6 by reading back through cached memory (#1376)
- **engine**: the demo recorder sees the whole world and a level horizon on an anchored mission (#1382)
- **ci**: stop asserting a 16.6 ms tick p99 on a runner whose own noise floor is 24 ms (#1383)

## [0.3.20] - 2026-08-25

### Changed

- **network**: one normalizeIp/extractIp for admission and admin commands (#1291)
- **engine**: one TOML required/optional field helper set for the seven def parsers (#1292)
- **engine**: one strict number/string parse family in engine/util/Parse.h (#1294)
- **engine**: one quaternion rotate family in engine/math/Quat.h (#1296)
- **engine**: one FNV-1a implementation on the true standard offset basis (#1295)
- **engine**: engine/math Angles.h and Units.h replace the scattered constants (#1293)
- **flight**: one angle-of-attack formula for HUD and haptics (#1300)
- **network**: lobby registration uses the one JSON escaper (#1299)
- **engine**: one whole-file read helper over IFilesystem (#1301)
- **entity**: one engine-failure flag decode for the four force models (#1302)
- **ai**: one steer-toward-point control tail; lead and lag pursuit share a file (#1303)
- **server**: command-refusal matching uses the constants that mint the strings (#1304)
- **network**: named quant/dequant pairs make the articulation wire contract testable (#1305)
- **renderer**: one sub-tick extrapolation helper for the six coincident sites (#1307)
- **engine**: the airport index adopts the shared little-endian codec helpers (#1308)
- **engine**: tick-rate literals derive from the TickRate authority (#1309)
- **game**: one menu navigation helper behind the six screens (#1310)
- **network**: one snapshot baseline cache for live play and replay (#1311)
- **engine**: one portable-socket compat header in platform/ (#1312)
- **renderer**: a HudElement builder replaces the per-screen boilerplate (#1313)
- **network**: engine/net internal dedup — forwarders, rate limiters, send loops, notices (#1314)
- **ai**: controller-family dedup — loiter, runway helpers, fire control, factory table (#1315)
- **server**: fl-server internals — printAdmin adoption, config read helper, UserConfig tables (#1316)
- **engine**: platform backend dedup — SDL3 keymap table, vulkan helpers, worker lifecycle (#1318)
- **audio**: PCM and content helper dedup — float→int16, builtin textures, cache keys (#1319)
- **game**: client UI small dedup — radio-menu skeleton, fading stacks, glue clusters (#1317)
- **tools**: tool dedup — terrain generators, pytest conftest, synchronous subprocess run (#1320)
- **engine**: serialization odds and ends — GLB writer, localtime shim, lowercase/trim (#1321)
- **tools**: one scaffold behind the twelve validator CLIs (#1325)

### Fixed

- **flight**: the server survives a malformed flight-model instead of crashing at first spawn (#1278)
- **network**: session and admin tokens draw full entropy instead of a 32-bit seed (#1279)
- **campaign**: CRLF-authored templates keep their whole header (#1282)
- **renderer**: pack textures pass the same PNG sanity guard as terrain and campaign images (#1284)
- **network**: the RCON wire header is little-endian by construction, not by accident (#1286)
- **mission**: mission reports stay valid JSON whatever the mission is named (#1280)
- **ai**: a Lua AI script gets the same capability surface however it is attached (#1287)
- **game**: applying settings no longer flips bloom on until the next launch (#1281)
- **mission**: validate-mission rejects the trigger refs the runtime cannot fire (#1285)
- **ai**: mission-attached Lua scripts reach the ATC service the admin path already had (#1326)
- **game**: frame-driven client systems integrate the real frame time, not an assumed 1/60 (#1328)
- **build**: the release scripts date a release in UTC, the clock git-cliff uses (#1331)

## [0.3.19] - 2026-08-19

### Added

- **campaign**: geodetic fills so a template can place an airborne object (#1223)

### Fixed

- **campaign**: a resumed campaign draws fresh sorties instead of repeating the same one every restart (#1225)
- **ai**: builtin:fighter flies real altitude instead of pos.y, so anchored missions keep their AI airborne (#1226)
- **tools**: gen_terrain_tiles reads tile-sized windows, so the documented global base build fits in memory (#1227)

## [0.3.18] - 2026-08-18

### Added

- **mission**: let a mission choose what the player takes off with, via loadout: on a player slot (#1214)
- **mission**: move the sandbox home to a real latitude, and let a mission say where it is set (#1215)

### Changed

- **ai**: permit generated audio in a shipped pack, under four conditions (#1201)
- **modding**: replace the unloadable units/*.toml format with the entity-def path (#1205)

### Fixed

- **render**: animate variable-geometry wings and report sweep in the world state (#1197)
- **ai**: cover altitude hold on a manoeuvring heavy, and correct ai.md's AoA bound (#1198)
- **ai**: elevate a ground SAM's launcher so its missile actually leaves the deck (#1207)
- **script**: resolve a pack script's require() inside the mounted pack, not the working directory (#1212)
- **ai**: launch a ground SAM's missile at the target its own controller picked, so the shot arrives (#1213)

## [0.3.17] - 2026-08-16

### Added

- **flight**: let a flight model declare its dynamic-pressure placard (#1185)
- **ai**: let an altitude-hold caller size the loop to a heavy airframe (#1190)
- **audio**: give the LSO, crew chief and wingman voice keys packs can record (#1192)

### Changed

- serve the documentation site from docs.fighterslegacy.org (#1177)

### Fixed

- **docs**: correct the wrong flight and voice keys in the user guide (#1175)
- **server**: stop fl-server crashing on the first pack aircraft spawn (#1179)
- **docs**: make the ai.md examples run from the guide instead of a copy of it (#1183)
- **tools**: range-check large aircraft instead of skipping them silently (#1184)
- **tools**: make fm-trim follow the wing-sweep schedule (#1189)
- **tools**: stop validate-mod rejecting two documented pack conventions (#1191)

## [0.3.16] - 2026-08-12

### Changed

- **engine**: one JSON escaper, writer set and reader for the whole engine (#1161)
- **engine**: make the match event log the one event bus instead of a recorder beside five hooks (#1162)
- **engine**: give the game loop an ordered list of systems instead of one hook and a lambda (#1163)
- **server**: stop capability enforcement depending on which admin frontend you arrived on (#1164)
- **server**: make every admin frontend visible and unlockable from one command (#1166)
- **server**: tell an operator which config keys a reload applied and which need a restart (#1167)
- **server**: make fl-server's shutdown order a contract the compiler enforces (#1168)
- **net**: take WorldBroadcaster's hooks and queries at construction instead of through 19 setters (#1169)
- **net**: move the connect gauntlet and handshake out of WorldBroadcaster into PeerAdmission (#1170)
- **net**: move the per-tick snapshot path out of WorldBroadcaster into SnapshotPipeline (#1171)
- **net**: move chat, voice, the scoreboard and the datalink into SessionComms (#1172)

### Fixed

- **atc**: retire a landed flight so it stops re-taking the runway (#1155)
- **atc**: stop a distant or non-landing arrival from blocking every departure (#1156)
- **server**: remove the persistent-world flag and lobby URL that pointed at nothing (#1158)
- **engine**: stamp every match event with the tick it happened on (#1159)
- **world**: assert FactionRegistry's thread-ownership tiers instead of documenting them (#1160)
- **platform**: route async HAL completions by request id so a second consumer cannot steal them (#1165)
...
- **content**: reject out-of-range priority values in mod manifests (#1130)
- **ci**: unblock the coverage gate and separate a threshold miss from a measurement failure (#1138)
- **build**: stop silently dropping GameNetworkingSockets where protobuf ships shared-only (#1139)
- **engine**: spawn load-test entities above their own terrain, not the origin's (#1142)
- **ai**: keep loitering aircraft in the air — coordinate the turn, limit the bank, hold the speed (#1144)
- **ai**: stop every station-keeping controller from rolling itself inverted (#1147)
- **ci**: stop a cancelled coverage run from filing a "the build is broken" issue (#1148)
...
- **tools**: windows validation environment for pre-push MSVC checks (#1115)
- **tools**: bbox-limited tile enumeration in gen_terrain_tiles.py (#1121)

### Changed

- audit the documentation against the binary and reorganize it around four personas (#1051)
- apply the 2026-07-30 strategic review to the roadmap and decision records (#1103)
- **modding**: reconcile weapons-sensors.md with the parsers (#1120)
- drain CLAUDE.md's Key Files registry into docs/ (#1124)

### Fixed

- **docs**: point site_url at the real published domain (#1055)
- **game**: stop single-player contending for the standard game port (#1057)
- **server**: report the server's uptime, not the machine's (#1058)
- **server**: enforce world.entity_soft_cap, with a reserve for players (#1059)
- **game**: make the binding table the authority for every gameplay control (#1060)
- **tools**: make the windows validation VM predict CI instead of itself (#1116)
- **tools**: probe UDP for a free port, and retry on bind failure (#1118)
- **engine**: read sensor eccm from [sensor], not the TOML root (#1119)
