...
- **game**: split the monolithic `GameImpl` into two cohesive sub-structs — `GameServices` (stable init-time systems that live for the whole program: platform, content, renderer/scene, HUD, console, screen manager, …) and `SessionContext` (per-session objects created in `startGame()` and torn down in `stopGame()`: `localServer`, `clientNet`, `clientHandler`, `hapticController`, `discoveryListener`, `inspector`, server thread + `serverReady`/`sessionFailure` atomics). Session lifetime is now an explicit object; no behavior change. `FlightScreenDeps` is retained as `FlightScreen`'s narrow interface, now populated from the two structs
- **game**: connection/startup failure signaling reworked from `std::atomic<const char*>` + static-string literals to a single typed `std::atomic<SessionFailure>` (new `game/fighters-legacy/SessionStatus.h`). `ClientNetEventHandler` maps `MsgConnectRefusal.code` → `SessionFailure` (dropping the per-handler `char[64]` reason buffer and its dangling-pointer caveat); `LoadingScreen` polls one atomic and renders via `sessionFailureMessage()` — the single point the i18n system will localize (#358). Same on-screen messages; `LoadingScreen`/`reinitLoading` lose their two `std::function<const char*()>` parameters
