...
- **tools**: **the free-port guard in front of `fl-server` probed the wrong protocol.** `free_port()` opened a `SOCK_STREAM` socket, took the port the OS assigned, closed it, and handed the number to a server that binds **UDP** — and TCP and UDP have separate port spaces, so a port free for one says nothing about the other. That is a `mission_harness_ci_smoke` failure (`bind failed: enet_host_create() failed`) on a PR whose entire diff was one line of YAML, on an 18-minute Windows job; a flake in a required check trains people to re-run red CI without reading it. The docstring asserted the collision-proof property as fact, which is why nobody re-read the code. It existed twice, verbatim, in `mission_test.py` and `record_demo.py`. Both now share `tools/common/fl_ports.py`, which probes UDP and — since the probe socket is closed before the server starts, so the choice is racy by construction and cannot be made otherwise from outside the process — **retries on a bind failure** rather than claiming a race it has not closed. `record_demo`'s log poll now distinguishes a bind failure from a timeout, so a taken port is retried in milliseconds instead of spending the full 15-second deadline and then blaming the demo (#1056)
- **tools**: **the Windows validation VM was predicting CI with the wrong compiler.** CI builds with Visual Studio 18 (2026), MSVC toolset `14.51.x`; provisioning installed VS 2022 (`14.44`), whose traditional preprocessor mis-tokenizes a raw string containing `\"` inside a `CHECK()` macro — so the `ci` tier failed to compile `tests/test_http_admin.cpp` with `C2017` on code that is perfectly fine and that CI compiles without complaint. A tool built to predict CI that invents failures instead is worse than not having it. It now installs the VS 2026 Build Tools, upgrades rather than skipping when an older Visual Studio is already present, and prints the compiler version with a loud warning when the major version does not match CI, so a red tier can be told apart from a mismatched VM (#1114)
- **tools**: the Windows VM's `vswhere` queries silently ignored the compiler that had just been installed. VS 2026 Build Tools register as a *prerelease* ("Insiders") instance, and `vswhere` omits those unless asked with `-prerelease` — so provisioning reported success, the summary reported VS 2022, and both were telling the truth about different questions (#1114)
- **tools**: the Windows VM never installed **OpenSSL**, so `cmake/dependencies.cmake` force-disabled GameNetworkingSockets and the `ci` tier built enet6-only. It is absent from the repo's `vcpkg.json` because GitHub's runner image ships it preinstalled — an implicit dependency on the hosted image that nothing recorded. The tier's own `FL_ENABLE_GNS` assertion is what caught it (#1114)
- **tools**: the Windows VM never installed the **Vulkan loader**. `vulkan-1.dll` normally arrives with a GPU driver and a headless guest has none; the SDK ships headers and tools without it, and mesa ships the ICD and driver without it. The game linked cleanly and then died at startup with `0xC0000135` (`STATUS_DLL_NOT_FOUND`), an error that names no file and points nowhere near Vulkan (#1114)
- **tools**: three stale-artifact defects in the Windows VM, all of the same shape — something cached passed the check in front of it and produced a confident wrong answer. `Get-RemoteFile` trusted HTTP 200, so a Bing search page (aka.ms answers an *unknown* alias with `200 OK`, not a 404) was written as `vs_buildtools.exe` and failed half an hour later as "the file or directory is corrupted"; its cache was then keyed on filename alone, so after the URL was corrected the old bootstrapper was still served and quietly installed the wrong compiler while the log said otherwise; and the `ci` tier believed a **cached CMake `NOTFOUND`**, which is never retried, so installing the missing dependency had no effect. Downloads are now validated against expected magic bytes as they land and cached against their source URL, and the tier purges the configure cache and retries once before trusting its own assertion (#1114)
- **tools**: the Windows VM's `runtime` tier reported bare exit codes with no diagnostic. It now captures the game's stdout/stderr and echoes them on failure — a check that cannot say why it failed costs more time than it saves (#1114)
- **docs**: `tools/windows-env/` claimed the WinRM gems ship with Vagrant. They ship with *HashiCorp's* Vagrant; Fedora's package patches in rubyzip 3.x while every published `winrm-fs` caps at `rubyzip ~> 2.0`, so the WinRM communicator cannot load at all and `vagrant up` dies with `cannot load such file -- winrm` before it even fetches the box. The README now says to install from HashiCorp's repo and warns that newer `vagrant-libvirt` defaults to `qemu:///system` — running `vagrant status` against a `qemu:///session` VM deletes its `.vagrant` id file, detaching Vagrant from an otherwise healthy machine (#1114)

- **game**: **the radio push-to-talk key also toggled master arm.** `V` was the default binding for both `PushToTalkPrimary` and `MasterArm`, and both were live while flying — so a player held `V` to talk, released it, and their guns and missiles stopped firing, with a HUD annunciator they had no reason to look at because they had touched no weapons control. Talking again toggled it back, which made it intermittent and close to impossible to attribute. `M` was double-bound the same way (`VoiceNetCycle` / `GmMap`) and was latent only because one of the two was never read. `V` stays with push-to-talk (the near-universal convention); master arm moves to `4`, joining the armament digit row, and the game-master map moves to `F12` (#1050)
- **game**: **master arm ignored its own binding.** `FlightInputCollector` read `input.isKeyDown(Key::V)` directly instead of resolving `InputAction::MasterArm`, so rebinding master arm did nothing — and rebinding push-to-talk off `V` did not clear the collision either, because master arm was not looking at the binding table at all. That was the one workaround a player would reach for. It resolves through its action now, like every other control (#1050)
- **game**: **nothing ever checked the shipped defaults for conflicts.** `InputBindings::conflictsWith()` existed and was unit-tested, but only ever validated a *user* rebind — `applyDefaults()` was the single binding set held to no rule. `findConflicts()` now walks the whole table, a test asserts the defaults are clean, and the game logs a warning at startup naming both actions when a hand-edited `bindings.toml` clashes. Six further live collisions fell out of turning the check on: `E` dispensed countermeasures *and* panned the free camera up; `K` was the airbrake *and* the crew-seat picker; `H` was the arresting hook *and* team chat; `B` was the wheel brakes *and* the second push-to-talk; `M` opened the aircraft manual *and* the GM map; and `Page Up`/`Page Down` were the throttle *and* the manual's scroll keys, on an overlay that deliberately keeps the aircraft flying (#1050)
- **game**: the binding table claimed `S`/`W`/`A`/`D` for pitch and roll while the game read the arrow keys, and claimed the arrows for the cockpit view pan at the same time — a double duty the controls page documented as a known annoyance rather than the two-live-actions-on-one-key defect it was. The arrows fly the aircraft; the cockpit pan is the keypad cross (#1050)
- **server**: **`[world] entity_soft_cap` was parsed, range-checked, stored — and never applied.** The pool mechanism it drives works (`EntityPool::alloc` refuses past the cap); nothing ever called `setSoftCap` with the configured value, so an operator who set a ceiling to bound memory against a runaway mission script or a stuck respawn loop got no cap, no warning and no way to tell from the outside. A silently ineffective resource control is worse than an absent one, because it is relied upon. The cap is now applied before anything spawns, and is hot-reloadable via `reload_config` — raising it is how an operator relieves a world that is refusing spawns, and waiting for a restart to do that is waiting through the outage (#1049)
- **server**: a pilot the server could not give an aircraft was **admitted anyway**, with a `MsgConnectAck` naming entity 0 — no camera anchor, no controller, no own-record in the snapshot, and no reason shown. The client sat on a loading screen. Every admission path now refuses instead: `ConnectRefusalCode::ServerFull` when the entity cap is binding (retryable — "try again shortly") and the new `NoAirframe` when `player_entity_type` names a type no loaded pack registers (an operator configuration fault, which the server log now names with its fix). A respawn in a full world stays **queued** rather than being silently consumed, and an `observer→pilot` role change is declined with the peer left an observer rather than promoted to a pilot with nothing to fly (#1049)
- **server**: the first line of every server log said `fl-server 0.0.1 starting` — a hardcoded literal, while `--version` and the MCP `serverInfo` both reported the real build. An operator reading logs to confirm which build is deployed got 0.0.1. It now stamps `FL_VERSION_STRING` (#1049)

- **server**: **`status` reported the host machine's uptime, not the server's** — through all five admin frontends (stdin console, RCON, the ENet admin channel, REST `GET /status` and the MCP `admin_command` tool), because they all dispatch the same command. `ServerCommandContext::env.startTime` was a value-initialised `steady_clock::time_point`, and every command handler captured the context **by value** 34 lines before `main` assigned it — so each closure kept a permanent copy of the clock's epoch, which on Linux (`CLOCK_MONOTONIC`) is boot. The number advanced at the correct rate and was plausible enough to be believed during an incident. Fixed at the class of bug rather than the field: the start instant is now a `ServerUptime`, a value that captures its own start at construction and therefore cannot hold the epoch, and one instance is created in `main` and handed to both frontends that report uptime — so `status` and `/health` agree by construction instead of by two implementations happening to match. `GET /health` no longer derives its own (#1048)

- **game**: **Instant Action and Free Flight failed to start with "Port already in use."** — and so did loading a mission from the CLI, since all three enter the same session path. The server browser's LAN discovery listener was created during startup for *every* session type, single-player included, and bound the wildcard game port 4778 for the entire process lifetime; it was only ever polled on the browser screen and never released. The embedded `fl-server` then could not bind `127.0.0.1:4778`, because enet6's `enet_host_create` sets no `SO_REUSEADDR`, so its bind failed and the client reported the failure verbatim. Regression from #1003. Fixed on three fronts: the client-provisioned single-player server now binds its own port (`LocalServer::kLocalServerPort` = **4776**) rather than competing for the standard one; the LAN listener is created on entering the server browser and destroyed on leaving it, so it holds a socket only while it is on screen; and the embedded server is spawned with the new `--no-discovery` flag, so a server serving one loopback player no longer advertises itself on the LAN or binds a query responder. 4776 rather than an adjacent port because fl-server derives an unconfigured query port as *game port + 1* — 4779 belongs to a dedicated server's query responder, and 4777 would derive its own query port straight back onto 4778 (#1054)
- **game**: a second LAN discovery listener was created inside every single-player session, asking for the wildcard game port the embedded server had already taken moments earlier. It therefore failed on every launch since it was written, logging a warning nobody acted on, and served no purpose — a single-player session has nothing to do with LAN servers. Removed (#1054)
- **game**: the client aborted with **"double free or corruption"** on exit from any single-player session. `~GameImpl` tears the platform down explicitly and `SDL3Window::shutdown()` ends in `SDL_Quit()`, which destroys every SDL audio stream — but voice capture was not in that explicit sequence, so it was left to `~VoiceChat` running afterwards as a member destructor, calling `SDL_DestroyAudioStream` on a stream SDL had already freed. Voice capture now closes alongside the music/SFX/warning-tone/engine-audio shutdowns, leaving the destructor a no-op. Pre-existing but unreachable until now: no single-player session could start, so nothing ever reached this teardown (#1054)
- **platform**: `Subprocess::stop()` returned immediately after `SIGKILL`/`TerminateProcess` without waiting for the process to actually exit. Signalling is asynchronous, so the dying server could still hold its socket when the next session tried to bind — a second, independent route to "Port already in use." on quit-to-menu-then-fly-again. `stop()` now waits for the child to be reaped, with a 2 s backstop (#1054)

### Changed

- **docs**: **the 2026-07-30 strategic architecture review lands.** The Critical Path is pruned to the one chain still open (four of its five chains had fully landed), Phase 4 gains the consolidation program that now precedes the gate — protocol hardening (#1063) and interface consolidation (#1064) as #1036 Stages 5–9, shipping as interim `v0.3.14+` releases — plus two new acceptance lines: the 128-client `reference` profile must pass **with sensor content loaded and datalink + voice active** (the committed numbers were measured against a hollow world, so the `O(P²)` datalink cost the review found was invisible to CI), and every client→server message type must be rate-limited or one-shot-gated with no amplified reply. The server-discovery acceptance line now states the client capability that actually ships rather than naming an fl-lobby service that exists in no repo (#999 stays Phase 5). Deployment artifacts move Phase 7 → Phase 5 (#160, #228, #1096) so a self-host product's hosting artifacts precede the fleet operator that consumes them, and every phase gate now files the incoming phase's execution-plan Task. Recorded as a dated decision record in `architecture.md` (#1066)
- **game**: **the binding table is now the authority for every gameplay control.** Keyboard, mouse, gamepad and the free camera all resolve through `InputBindings`; there is no raw `isKeyDown(Key::…)` or `SDL_GetKeyboardState` read left in the flight, camera, replay, photo-mode, overlay or console paths. A control the table does not own is a control the player cannot rebind and the conflict checker cannot see, which is how `V` came to drive two live actions. Consequences: every control on the [controls page](docs/user-guide/controls.md) is rebindable and gamepad-capable, including the free camera, the console toggle and the performance overlay; the collector's twelve hand-rolled edge detectors (two of them duplicated per action, one for the key and one for the pad button) collapse into a single `ActionEdgeTracker`; and the `Shift+C` canopy chord is gone, because the table cannot express a chord and a chord whose modifier is itself a bound action (`LeftShift` = max throttle) cannot be made to behave (#1050)
- **game**: `config/bindings.toml` holds an **arbitrary-length list of bindings per action**, all live at once, under a single `[bindings]` table. The `[primary]`/`[secondary]`/`[gamepad]` slots #1050 introduced (themselves replacing `[primary]`/`[alt]`) only ever existed *because* there was no list — three named pigeonholes were a storage shape leaking into the file format, and they could not express "the gun is Space, left mouse, a shoulder button, joystick 1 button 5 **and** joystick 2 button 7". Order in the list is the player's priority for analog axes; for a digital control any one being down is enough. A file written by an older build still loads — the legacy sections are read in slot order into one list per action, so rebinds survive the upgrade (#1050, #1061)
- **game**: **a binding names its device, by GUID.** SDL renumbers device indices on hot-plug, so an index-keyed joystick binding silently migrates to a different stick the first time one is unplugged — `IJoystick::getJoystickGuid`'s own comment already said it was "for persistent binding saves so bindings survive device reconnect", and the binding table had simply never taken it up. Two joysticks with different GUIDs are now addressed independently and swapping their USB order moves neither binding. A shipped default names no device, meaning "whichever stick is plugged in", because the default table is compiled before any device exists (#1061)
- **game**: **the four HOTAS axes moved out of `user.toml` and into the binding table.** They were the last parallel input-config path in the game: `[controls] hotas_*_axis`, read by index on device 0, invisible to `InputBindings` and to the conflict checker — the same defect class #1050 exists to remove, surviving in the one corner that fix did not reach. `ControlsSettings` now has no input fields left, only force feedback. The old keys are read once on upgrade and folded into `bindings.toml` (axis assignment, deadzone, inversion, and an axis disabled with `-1`), then never written again; your previous file is kept as `bindings.toml.bak` (#1061)
- **game**: per-axis tuning is keyed by **(source, device, axis index)** rather than by one of six `GamepadAxis` values, so "joystick 2, axis 9" is expressible and two identical sticks can be tuned apart. `[[axis_config]]` gains a **`mode`**: `Centered` for a spring-return stick, `Absolute` for a lever that stays where you leave it. That distinction used to be hardcoded — the HOTAS throttle got its `(raw + 1) / 2` remap in a separate code path, and the docs had to tell players `invert` "is not meaningful" for a trigger axis. An `Absolute` axis is also *always* in command: a throttle closed to idle reads 0.0 and is a command, not an absent input, so the keyboard throttle no longer fights a stick the player deliberately pulled back (#1061)
- **game**: `bindingDown`/`actionDown` and friends take an `InputSources` struct — the live IInput, IJoystick and device table in one value — instead of a parameter per HAL. `bindingDown(in, b, gamepadId)` was already three arguments before #1061 added a second HAL and a GUID map; threading four or five through `FlightInputCollector`, `CameraInput`, `FlightScreen` and `Game.cpp` would put the same call-site edit in front of every future device class. Every pointer may be null, and null is normative: that hardware is absent, so its bindings are inert (#1061)
- **game**: `bindings.toml` is **version 3**. A version-2 file is converted in place — its bindings are preserved, its axis tuning is preserved, and the HOTAS axes are folded in from `user.toml` — while a version-1 file is still discarded in favour of the current defaults, because in *that* map the defaults themselves were the defect (#1061)
- **game**: `InputAction::FireMissile` is renamed `FireStore` — it releases whatever is on the selected station, which may be a bomb, a rocket pod or a missile. `InputAction::Menu` is removed (nothing read it) (#1050)
- **server**: the admin command context is **shared, const and frozen at registration**. `registerServerCommands` takes a `std::shared_ptr<const ServerCommandContext>`; assigning a context field after registering is now a compile error rather than a silent no-op, which is the shape that produced the `status` uptime defect. It also stops deep-copying a struct of eight `std::function`s and three strings into each of the ~50 handlers, and again into every `enqueueSimCallback` lambda they build — those copies are now a refcount increment (#1048)

### Added

- **tools**: `tools/windows-env/` — a Vagrant/libvirt Windows Server VM that runs the real MSVC toolchain against a local branch, so a Windows-only failure surfaces before the push rather than after it. The local warning sweep is a g++ *proxy* for MSVC, and it has been wrong repeatedly at real cost: C4456 (shadowed local, #1037), C4244 (narrowing, #1041) and `<string_view>` not providing `<string>` (#1007) were each clean on Linux. Three tiers mirror what CI does — `ci` reproduces the `ci.yml` Windows leg (`debug-msvc` under vcpkg, the `FL_ENABLE_GNS=ON` cache assertion, build with warnings-as-errors, `ctest`), `smoke` reproduces `scale-gate.yml`'s `windows-smoke`, and `runtime` adds what CI has never had on Windows: it renders `builtin:shape-gallery` and asserts a screenshot came back, exercising window and swapchain creation, shader loading and the embedded `fl-server` handshake. Optional by design — nothing requires it and no hook invokes it. It is also **not** a measurement environment: it is sized off the host rather than pinned and renders through software Vulkan by default, so `reference-env` remains the only instrument any performance number may come from (#1114)
- **tools**: an optional GPU passthrough profile for that VM (`FL_WINENV_GPU=1`), so the runtime tier can render on the production driver instead of lavapipe. libvirt attaches the card with `managed="yes"`, so it is reattached to the host the moment the VM stops — the host gets its GPU back for inference without a reboot or a static `vfio-pci` bind. The one-time host prerequisites (VT-d, an additive GRUB edit, moving the host display to the iGPU, and an IOMMU-group check) are a documented manual runbook, deliberately not automated (#1114)
- **ci**: `windows-env.yml` — a `workflow_dispatch`-only job that runs those same tiers on an optionally-registered self-hosted Windows runner, gated on the canonical repository and a `FL_WINDOWS_RUNNER_READY` repo variable. It follows the same security model as the reference runner (#569): a self-hosted runner on a public repo must never execute fork-PR code, so there is no `pull_request` path to it and the required Windows checks stay on hosted runners. The PR-path CI surface is unchanged (#1114)
- **game**: **a HOTAS button, axis or hat direction can be bound to any action.** `BindingSource` gains `JoystickButton`, `JoystickAxis` and `JoystickHat`. Before this, `IJoystick` was a fully implemented peer HAL — `getButtonCount`, `isButtonDown`, `isButtonJustPressed`, hats, GUIDs — that **nothing in `game/`, `engine/` or `tools/` ever called** except one block reading four axes by index. A HOTAS trigger could not fire the gun on any binding, on any device, at all. One action can now hold keyboard, mouse, gamepad and a button on each of two distinct joysticks simultaneously; a unit test drives exactly that five-binding gun (#1061)
- **game**: POV hats get a real rising edge. `IJoystick` reports a hat's position and offers no just-pressed flag for it, so a hat could otherwise only ever drive *held* controls — bind the landing gear to your POV and nothing would happen. `JoystickDevices` keeps the previous frame's position per device and derives the edge from it. A binding on a cardinal direction also fires on the two diagonals either side, so a four-way view control does not go dead when a thumb rolls off centre; a binding on a diagonal is exact (#1061)
- **game**: `bindings.toml` records the joysticks it refers to in a `[[devices]]` table, GUID plus the name the device reported, written the first time each one is connected — and the game logs every device arriving and leaving with its GUID. A GUID is 32 hex characters; without the name beside it, a warning about an absent device is unreadable, and without the GUID in the file there is no way to hand-edit a binding onto a specific stick (#1061)
- **game**: **a binding whose device is absent is preserved, inert, and logged once per device.** It is not pruned on save — silently destroying a player's HOTAS map the first time they launch without the stick plugged in is the worse failure — and it is not silent, because a control that stopped working with no signal is the same "intermittent and impossible to attribute" experience that made #1050 expensive. Plugging the stick back in restores the control with no re-binding (#1061)
- **input**: `findConflicts()` covers joystick bindings, and the device comparison is deliberately the *overlap* test: two bindings on different GUIDs are different inputs even at the same button number, while an "any device" binding overlaps everything, because at runtime it can resolve to any of them. The shipped defaults — which now include four joystick axes — stay clean (#1061)
- **input**: `engine/input/InputNames.{h,cpp}` holds the enum ⇄ name tables. Bindings and axis configs both name a `GamepadAxis`, and once the axis-config table was re-keyed by `(device, axis)` it needed the same names `InputBindings` serializes; a second copy of that list would have been the #1050 defect one level down (#1061)

- **game**: input actions carry a **context** — the set of session modes (flying / spectating / replay / photo) in which the game reads them — and two actions conflict only when those sets intersect. Without it a conflict checker has to choose between flagging every legitimate reuse (`Space` is the gun while flying and the pause key while watching a replay, and they can never both be live) and checking nothing, which is what shipped. Twenty-nine gameplay controls that had no action at all are now in the table: max throttle, wheel brakes, radar mode, countermeasures, ECM, the arresting hook, the canopy, the comms menu, the aircraft manual and its scroll, the crew-seat trio, the free camera's nine, the console toggle, the performance overlay, the spectator picker, the replay transport and photo mode (#1050)
- **input**: the `Key` enum gains the punctuation and keypad blocks (`, . / ; ' [ ] \\ \`` and the full numeric keypad). The shipped map had run out of letters — the comment above one default read *"Every letter is spoken for"* — and running out of keys is how two actions end up on one (#1050)
- **ci**: a sixth `docs-drift` check, **`input-actions`**, diffs the `InputAction` enum against the key map in `docs/user-guide/controls.md` both ways. #1050 was found by diffing the documented key map against `applyDefaults()` **by hand**; the key map is the one page that goes stale the moment an action is added (#1050)
- **engine**: the entity soft cap is **two-tiered**, and players hold the upper tier. A flat first-come-first-served ceiling fails in exactly the situation it exists for — the things that fill a world fastest (projectiles, AI, a runaway script) are the things the cap is meant to bound, and once they hold the last slot the next casualty is a human who cannot join or respawn. `EntityPool::alloc` now takes a `SpawnClass`, and `max_peers` entities are held back as a **player reserve** that only a pilot's airframe may allocate from. The reserve is derived from `max_peers` rather than configured (there is one correct value for it) and clamped to half the cap, so a large `max_peers` against a small cap cannot leave the world with room for nothing but airframes (#1049)
- **server**: cap refusals are **countable and reported**, because a spawn that returns null with no trace is undiagnosable from an operator's seat — which is how the unwired key survived. `EntityManager` counts every refusal and emits an aggregated Warn at most once per 10 s (per-spawn logging would be a flood at the exact moment the server is already under pressure); `status` shows `entities: N/cap` plus `cap refusals: N`; `--metrics-json` carries `entity_soft_cap` and `entity_cap_refusals` (additive, no schema bump); and the headless `--mission-report` carries the refusal count, with `tools/mission_test` **failing** on a non-zero value — a run whose spawns were truncated by a cap is not a mission result, and every count above it would be measuring the cap rather than the mission (#1049)

- **server**: `fl-server --no-discovery` forces both LAN-facing sockets off — the discovery beacon and the server-info query responder — regardless of `[discovery]` config. Lets a server run with no LAN presence at all; used by the game client for its embedded single-player server (#1054)
...
- **docs**: operator configuration reference corrected against the v0.3.13 binary (#1047). `[trace] input_trace_dir` was documented inside the `[replay]` section, so the one key that enables input tracing appeared to belong to a section that does not parse it; the FLIT record size was stated as 28 bytes and is 32. `[bots]` was entirely undocumented — including that backfill silently does nothing without teams. `[flight]` documented 1 of its 10 keys, `[discovery]` neither of its query-responder keys, `[world]` neither `test_spawn_entity_type` nor `entity_soft_cap`, and the `respawn`/`spectate` admin commands were absent from the command reference. `ai.provider.api_key` is now documented as the rejected key it is. `[world] entity_soft_cap` was documented as **parsed but not enforced**, which is what the code did — the audit's finding, fixed in this same release by #1049 and re-documented against the enforcing behaviour

### Added

- **tools**: `code_stats.py` — application statistics for a milestone gate. Reports composition by **category** (production code, test code, documentation, configuration, build system, data, test fixtures, media) rather than by language alone, plus the product surface: wire messages, config keys, admin commands, Lua bindings, CLI tools and test counts. Text is measured in lines, binary and bulk data in files and bytes, because a line count of a fuzz seed or a 134k-row airport table describes nothing anyone wrote. Surface counts are imported from `docs_drift.py` rather than re-implemented, so the release notes and the drift gate cannot disagree. An unclassified file type is a hard error: silent under-reporting that still looks like a real number is the failure mode of a statistics tool (#1047)
- **ci**: `stats.md` and `stats.json` attach to every release, and the statistics block is appended to **minor** (`vX.Y.0`) release bodies by a separate `Release statistics` workflow. Separate because the hand-authored notes land *after* `release.yml` completes and replace the body wholesale — the timing race documented in the release-notes gate — so a block written at publish time would be erased by the first edit. It rewrites only text between its own markers, and refuses to write into an empty body (#1047)

### Added

- **docs**: the documentation is reorganised around **four personas** — [User Guide](docs/user-guide/index.md), [Server Ops Guide](docs/server-ops/index.md), [Modding Guide](docs/modding/index.md) and [Developer Guide](docs/developer/index.md) — and published as a **mkdocs-material site** via GitHub Pages. `mkdocs build --strict` gates every PR: a broken link, a broken heading anchor or a page missing from the nav fails the build, which is the structural fix for the two divergent hand-maintained indexes that let the docs drift in the first place. There is one index now, `docs/index.md`, and the root README routes to it rather than enumerating pages (#1047)
- **docs**: a **User Guide** exists for the first time — installation and system requirements, a five-minute quick start, the full controls reference, wingman and voice, replays and photo mode, and multiplayer. Player-facing material was previously only inside a document titled "Sandbox Reference" filed under developer docs (#1047)
- **docs**: new operator references — `admin-api.md` (REST endpoints, bearer-token auth, the health probe), `mcp-agent-surface.md` (the agent surface and the three independent checks that bound it, closing #603), and `metrics.md` (every `--metrics-json` field and how to triage it, the catalog half of #551). All three cover surfaces that shipped in v0.3.11–13 with no reference documentation at all
- **docs**: new `modding/manifest-reference.md` (the pack manifest schema had been described in three places, none complete) and `developer/tools.md` (a catalog of all 21 CLI tools; 17 were undocumented)
- **docs**: the 11 frozen spike resolutions moved to `developer/decisions/` and each carries a status banner saying it is history rather than a guide, so a superseded design can no longer be mistaken for current behaviour

### Fixed

- **docs**: the wire-protocol message table was missing `GmWorldState`, the three voice messages, and the `ConnectIdentity`/`ConnectJoinPassword` extension tags; `ServerQuery`/`ServerInfo` were listed as "pending" although the query responder ships and runs by default (#1047)
...
- **docs**: `docs/developer/replay-format.md` — the `.flrep` replay file specification, versioned for real because a replay outlives the build that wrote it (#187)
...
- **tools**: `measure_windows.ps1` invoked a bare `python`, which on a stock Windows box resolves to
  ...
  default build directory falls back to `build\release-msvc` (the preset `docs/developer/development.md` tells
...
- **netcode**: granted-authority ConnectAck extension and client affordance gating (#949, epic #944).
  ...
  commands its capabilities permit through the empty-token grant channel. `docs/developer/network-protocol.md`
...
- **platform/game**: force-feedback joystick effects via SDL3 haptics (#928, Epic #587). `IJoystick`
  ...
  `ffb_strength`; `docs/developer/haptics.md` extended. Covered by a `TrackingJoystick` in
...
- **engine/game**: padlock/target-cycle input actions and camera-key unification (#689, Epic #587).
  ...
  entries. Keymap documented in `docs/user-guide/controls.md`; covered by new cases in `tests/test_input.cpp`.
...
- **network**: fl-lobby registration client and in-game server browser (#143, Epic E #497). The HTTP HAL
  ...
  `docs/server-ops/lobby-api.md` (the fl-lobby Go service, #999, is written from it). Covered by
...
- **docs**: game-mode authoring guide (#525, Epic E #497). New
  ...
  (`[match] mode` / `mission@mode`), the match lifecycle, and `validate-mode`. `docs/server-ops/server-config.md`
  gains the `[match]` section (mode / end_screen_s / reconnect_grace_s) and documents the now-enforced
  `[server] password`; `docs/developer/network-protocol.md` documents the new Epic E message ids, the raw-UDP
...
- **mission**: per-epic cinematic demo missions — one per engine epic landed for v0.4.0 (#909).
  ...
  `docs/developer/demo-recording.md` documents the set + the roster-id/fixed-point `look_at` rule, and
...
- **ci/docs**: `demo-videos` workflow + release-runbook step (#918). A `workflow_dispatch`-only CI job
  ...
  and `docs/developer/demo-recording.md` document the flow. The only always-on CI addition remains
...
- **tools/mission**: `record_demo.py` driver + the v0.4.0 demo mission set (#917). `missions/demos/`
  ...
  `docs/developer/demo-recording.md`.
...
- **ai**: Expanded the `intent` eval suite to 111 utterances against the shipped six-command wingman grammar (16 per command + 13 out-of-grammar `unknown` + 5 prompt-injection cases); noted in `docs/developer/decisions/ai-provider-evaluation.md` that the shipped grammar is the same size as the placeholder (so the #769 "shorter grammar" latency lever did not materialize) and that re-measurement on the expanded suite is pending a model + reference hardware (#781)
