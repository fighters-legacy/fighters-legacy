...
- **audio**: OGG Vorbis decode replatformed from vendored **stb_vorbis** onto the reference **libogg v1.3.6 + libvorbis v1.3.7 (vorbisfile)**, FetchContent-pinned and statically linked (closes #723). Content-pack audio is attacker-controlled and the #709 fuzzing batch proved stb_vorbis — a trusted-input decoder — is not memory-safe on malformed streams (SEGV in its own `vorbis_deinit`/`setup_free` cleanup, integer-overflow wild allocations; both reproduce on current stb master), while a subprocess sandbox would contradict the ratified RFC #83 in-process containment decision and cannot make the in-process fuzz target clean. The `fl::decodeOgg`/`openOggStream` API is unchanged for callers; the rewritten `OggDecoder.cpp` adds defense-in-depth on top of the hardened decoder: a full-decode output cap (`kMaxDecodedSamples`, 64 MiB PCM — decompression bombs are rejected, never truncated), a channel/sample-rate sanity envelope (1–8 ch, 8–192 kHz) at open, a bounded consecutive-`OV_HOLE` retry so an adversarial stream cannot spin the main thread, and a chained-bitstream format-change guard so format-switched PCM never reaches the audio backend. `engine/audio/ogg_impl.{c,h}` (the single-TU C stb_vorbis wrapper, the codebase's one global-namespace exception) is deleted — `MusicManager` now drives the typed `fl::` streaming API via a new RAII `OggStreamPtr`; stb stays vendored for stb_image only (#723)

### Added

- **test**: `fuzz_ogg` harness re-enabled — moved back from `fuzz/disabled/` into the per-PR `fuzz-smoke` gate and the weekly deep run (auto-enrolled by the `fuzz/fuzz_*.cpp` enumeration), now that the OGG decode path is memory-safe on adversarial input (see the libvorbis replatform above). Ships minted seeds (`seed-silence.bin` from the shared `tests/ogg_fixture.h` `kMinimalOgg`, plus a truncated-first-page variant) and a `fuzz/fuzz_ogg.dict` (OggS capture pattern + the three Vorbis header-packet signatures); `fuzz/disabled/` is now empty and keeps only the generic parking-convention README. New adversarial Catch2 cases in `test_music_manager` cover the OggS-prefixed-garbage shape that passes `AssetValidator`'s magic gate, truncated streams, the decode cap, EOF/rewind streaming behaviour, and `MusicManager`'s graceful open-failure path (#723)
- **build**: configure-time module-boundary layering guard (#713, closes #559). The seed `fl_assert_zero_dep()` in `cmake/layering.cmake` grows into `fl_assert_layering()` — armed from the root `CMakeLists.txt` via `cmake_language(DEFER CALL ...)` so it runs after every `add_subdirectory()`, it walks `LINK_LIBRARIES`/`INTERFACE_LINK_LIBRARIES` transitively (aliases resolved, `$<LINK_ONLY:...>` stripped) and fails configure with the full offending link chain on four rule classes: no `engine-*` target reaches a client/transport backend (platform-sdl3/vulkan/openal/enet/gns/net or the third-party libs behind them); `engine-protocol` links nothing beyond stdlib/`Threads::Threads` (the direct zero-dep check folded in); `fl-server` reaches no client backend; and the game binary + `fl-server` link the `platform-net` facade only, never `platform-enet`/`platform-gns` directly (locks in the #507 HAL-leak fix). The walker self-tests against synthetic targets on every configure so a broken graph walk cannot silently disable enforcement. The CI "Verify static linking" `ldd` backstop on `fl-server` broadens from `sdl3` to `sdl3|vulkan|openal`, and the policy (layer DAG, allowed edges, enforcement mechanics) is documented in a new "Module Boundary Policy" section + dated decision record in `docs/architecture.md` (#713)
- **test**: libFuzzer infrastructure and the first network-codec harnesses (#708, Sub 1 of #94). New top-level `fuzz/` scaffold with an `fl_add_fuzzer()` CMake helper (harness exe hung off a `fuzzers` umbrella target + a 60 s `fuzz_smoke_<name>` ctest, label `fuzz`, replaying committed `fuzz/corpus/<name>/` seeds plus a build-tree working corpus, auto-detecting `fuzz/<name>.dict`), gated by a new `FL_BUILD_FUZZERS` option and a clang-only, Linux-only `fuzz` CMake preset (`-fsanitize=fuzzer-no-link,address,undefined`, GNS/Vulkan disabled). Three pure, zero-mock harnesses: `fuzz_snapshot_codec` (`fl::decodeRecord`/`BitReader` over a `MsgWorldSnapshot` bitstream + TLV block), `fuzz_wire_tlv` (`fl::findExt`/`readExtValue`), and `fuzz_rcon_packet` (`fl::rcon::decodePacket`) — the first two link the zero-dep `engine-protocol` seam, the third compiles `RconServer.cpp` directly. Seeds are minted reproducibly from the unit-test builders by an in-tree `fuzz/mint_seeds.cpp` (`fuzz-mint-seeds` target). CI adds a per-PR `fuzz-changes` → `fuzz-run` (matrix per harness) → stable `fuzz-smoke` required check that auto-enumerates harnesses and stays green when no fuzzable source changed. Adds the `fuzzing` label, a `fuzz/corpus/**` REUSE annotation, and a `## Fuzzing` section (build/run/add-a-harness + minimize→fix→`regress-*.bin`+Catch2 finding policy) in `docs/development.md` (#708)
- **ci**: weekly deep fuzz run — `.github/workflows/fuzz-deep.yml` (#710, Sub 3 of #94). Runs **30 minutes per harness** every Tuesday (cron offset from the Monday CodeQL sweep) + on `workflow_dispatch`, layered on the per-PR 60 s smoke from #708. Auto-enumerates `fuzz/fuzz_*.cpp` into a matrix (one job per harness, `fail-fast: false`) so new harnesses are picked up with no workflow edit; each job builds its target under the `fuzz` preset, deep-runs it over the committed corpus + `.dict`, and on a finding fails red and uploads the reproducer as a per-harness `fuzz-deep-findings-*` artifact. A final `report` job (`permissions: issues: write`) then auto-files or extends an open `fuzzing`-labeled issue (`fuzz: scheduled deep-run findings`) linking the run and restating the minimize→fix→`regress-*.bin`+Catch2 finding policy. Matrix architecture is a deliberate divergence from the fighters-codex single-job reference — 13+ harnesses × 30 min would exceed GitHub's 6-hour job cap. Adds a "Weekly deep run" note to the `docs/development.md` Fuzzing section (#710)
- **test**: message-handler and content-parser fuzz harness batch (#709, Sub 2 of #94). Ten new libFuzzer harnesses on the #708 scaffold: two **stateful handler** harnesses over the `tests/` mocks — `fuzz_server_msg` (`WorldBroadcaster::onReceive` — ClientInput/AdminCommand/Heartbeat) and `fuzz_client_msg` (`ClientNetEventHandler::onReceive`, the widest untrusted client surface: all 10 server→client message types incl. the snapshot bitstream, TLV block, and chunk reassembly), both driven through a new `fuzz/FuzzFrames.h` length-prefix splitter so one input replays several `onReceive` packets in order and reaches the multi-packet states — plus eight **content/asset parser** harnesses (`fuzz_asset_validator`, `fuzz_terrain_png`, `fuzz_flight_model_toml`, `fuzz_entity_def_toml`, `fuzz_playlist_toml`, `fuzz_server_config_toml`, `fuzz_mod_manifest`, `fuzz_mesh_json`). Each ships synthetic seeds minted reproducibly by the extended `fuzz/mint_seeds.cpp`, plus a `.dict` for the text formats; the `kMinimalOgg` fixture is promoted to `tests/ogg_fixture.h`. Vendored-parser benign-UB idioms (toml++ `TOML_ASSERT`/`unreachable`, stb `buf+n` reader + zero-length `memcpy`) are quieted **for the fuzz build only** via a no-op `TOML_ASSERT` + `-fno-sanitize=unreachable,pointer-overflow,nonnull-attribute` in the root `CMakeLists` `FL_BUILD_FUZZERS` block — ASan and every other UBSan check stay on. An OGG/stb_vorbis harness is parked in `fuzz/disabled/` because stb_vorbis has **real** memory-safety defects on malformed audio (a SEGV in its own cleanup) that need decoder hardening/sandboxing first (tracked as #723; see `fuzz/disabled/README.md`). The `fuzz-smoke` matrix auto-enumerates the new harnesses with no workflow change. See `docs/development.md` (#709)
- **ci**: `.github/workflows/add-to-project.yml` — auto-adds newly opened (or reopened) issues and pull requests to the org-wide [Fighters Ecosystem project board](https://github.com/orgs/fighters-legacy/projects/3) via `actions/add-to-project`; sub-issues are covered both by this workflow (they open as ordinary issues) and by the board's built-in sub-issue workflow. Uses `pull_request_target` so fork PRs can read the `ADD_TO_PROJECT_PAT` org secret without checking out untrusted code. The same workflow lands in `fl-base-pack` and `fa-bridge` so all three repos feed the shared board
...
- **ci**: self-hosted reference-runner strict tier for the scale gate (#569). The `reference-gate` job now targets a repo self-hosted runner (labels `self-hosted, linux, x64, fl-reference`) registered on the existing 8-vCPU/16 GB `reference-env` libvirt VM, and runs with `--strict` **unconditional** — so the `≤ 16.6 ms p99` server-tick gate hard-fails at 128 clients instead of staying advisory on hosted runners. Provisioning is infrastructure-as-code under `tools/bot_swarm/reference-env/`: `setup-self-hosted-runner.sh` (dedicated unprivileged `flrunner` user, non-ephemeral registration, systemd service), a `runner-hardening.conf` drop-in (`ProtectSystem=strict`/`ProtectHome`/`PrivateTmp`/`NoNewPrivileges`), a `runner-job-cleanup.sh` per-job workspace-reclaim hook, and an optional `Vagrantfile` provisioner (`FL_REFENV_RUNNER=1 FL_RUNNER_TOKEN=… vagrant up`). A cheap hosted `select-profiles` job expands the dispatched profile (`reference`/`soak`/`entity-scale`/`nightly`) into the run matrix. **Security** (public repo + self-hosted): the job is `workflow_dispatch`-only behind the `fighters-legacy/fighters-legacy` guard plus a `FL_REFERENCE_RUNNER_READY` repo-variable on/off switch — no fork/`pull_request` path reaches the runner, and the VM is the sandbox. Scheduled cron is deferred (the runner box isn't 24/7 and GitHub skips missed crons), preserved as commented crons for a one-line re-enable. See `tools/bot_swarm/reference-env/README.md` (#569)
...
- **ci**: the scale gate's strict `≤ 16.6 ms p99` server-tick assertion moves from **advisory** (hosted runners, `--strict` only under a `workflow_dispatch` toggle) to **enforced** — the `reference`/`soak` profiles run on the self-hosted 8-core reference runner with `--strict` unconditional. The old `strict` boolean dispatch input is dropped, replaced by a `profile` choice; the `reference-gate` job is now `workflow_dispatch`-only (manual) with the deferred nightly/weekly crons commented for later re-enable (#569)
- **platform**: filesystem I/O moved to a new SDL-free `platform-stdfs` HAL backend (`StdFilesystem` + `StdAsyncFilesystem`, over `std::fstream`/`std::filesystem`), and the `SDL3Filesystem`/`SDL3AsyncFilesystem` backends were removed. `platform-stdfs` is now the single filesystem backend for the whole project: the headless `fl-server` links it instead of `platform-sdl3` (dropping its last windowing-library dependency), and the GUI client uses it too — SDL remains only for path *resolution* (`SDL_GetBasePath`/`SDL_GetPrefPath`) and windowing. Paths are opened from `std::filesystem::path` objects so UTF-8 stays correct on Windows. Async behaviour (worker thread + `service()` swap-drain, cancellation, empty/error semantics) is identical. The CI "Verify static linking" step now also asserts `fl-server` links no SDL3. Covered by `test_async_filesystem_std` (the former `test_async_filesystem_sdl3`, retargeted). First of #559's remaining layering sub-tasks (#711)
- **build**: extract the wire protocol into a zero-dependency `engine-protocol` CMake module. `SnapshotCodec.cpp` and the header-only protocol files (`GameProtocol.h`, `WireCodec.h`, `BitStream.h`, `Quantization.h`, `SnapshotCodec.h`, `AckWindow.h`) now build as a standalone static library that reaches only the C++ stdlib, so the game client, `fl-server`, `bot_swarm`, `net_check`, and the codec unit tests can consume the protocol without transitively pulling `WorldBroadcaster`/`engine-entity`. Files stay in `engine/net/` (no include-path change); `engine-net` PUBLIC-links `engine-protocol` so existing consumers are unaffected. A new `cmake/layering.cmake` seeds an `fl_assert_zero_dep()` guard that fails configuration if `engine-protocol` ever links anything but the stdlib — a first step toward the engine/client/server layering boundaries in #559 (#712)
- Update all `fa-content` references to `fa-bridge`, following the plugin repo's rename (fighters-legacy/fa-bridge#45, "it provides no content; it bridges the user's FA install"): `.gitignore` now ignores `mods/fa-bridge/`, the `docs/architecture.md` naming convention becomes `<game>-bridge`, `docs/distribution.md` notes fa-bridge's distribution positioning is an open decision (fa-bridge#37), and the crash-reporter test fixture mod id is renamed
...
- **renderer**: HUD and menu text is now truly horizontally centered at every resolution (#439). `HudElement` gains a `HudAlign align` anchor field (Left/Center/Right, default Left = no behaviour change) resolved renderer-side in `VkRenderer::recordOverlayPass` from the string's glyph count — the only resolution-correct place, since the glyph advance is in pixels. New `fl::countUtf8Codepoints` in `platform/Utf8Decode.h` and a constexpr `hudAlignOffsetPx` + exported glyph-cell constants (`kHudGlyphWidthPx`/`kHudGlyphHeightPx`) in `platform/RenderTypes.h`. All menu screens (MainMenu/Pause/Debrief/MissionSelect/MissionBrief/Settings), `FlightHud` (HDG + damage warning centered at 0.5, clock right-aligned at 0.98), and the `ServerNotice` banner adopt it, replacing the eyeballed magic x-offsets; `SettingsScreen` rows become a right/left column pair split at screen center (labels right-aligned at x=0.48, values left-aligned at x=0.52) and its mouse-hover hit-test now shares the drawn per-row y table with `buildElements()` (fixes hover drift on the lower rows). Loading screen, game console, and the F3 debug overlay stay intentionally left-aligned. Covered by new `test_hud_align` + alignment cases across the screen/HUD/UTF-8 suites (#439)
...
- **security**: memory-safety and DoS hardening surfaced by the new fuzz harnesses (#709, part of #94). `ClientNetEventHandler` now NUL-terminates the `MsgEntityTypeDef` wire `char[]` fields (`id`/`mesh`/`dmgMesh`) before treating them as C strings, fixing an out-of-bounds read a malicious server could trigger by sending fully-populated (non-terminated) fields. `WorldBroadcaster` rejects NaN/Inf in client control input before it enters the sim (`std::clamp` passes NaN through unchanged), so a spoofed `MsgClientInput` can no longer propagate NaN and trip UB at the float→uint8 throttle-telemetry cast during snapshot assembly. `WeatherController` clamps an out-of-range `MsgWeatherState.preset` to Clear instead of indexing past its 7-entry preset table (OOB read). `decodeTerrainChunkPng` now requires the PNG signature (no stb_image multi-format auto-probe into its more fragile PSD/PNM/HDR decoders), rejects any PNG whose chunk lengths overrun the buffer, and caps declared dimensions — closing a memory-exhaustion DoS where a ~100-byte content-pack chunk could force a multi-hundred-MB stb_image allocation. `parseServerConfig` tolerates a null logger on its parse-error / validation-warning paths (the pointer is documented as optional). Each fix ships a Catch2 regression test and a committed `regress-*.bin` seed. Also guards a benign zero-length `memcpy(nullptr, _, 0)` in the `MockFilesystem` test double (#709)
