...
- **game**: `config/bindings.toml` has **three slots per action** — `[primary]` and `[secondary]` for keyboard/mouse, `[gamepad]` for the pad — replacing `[primary]`/`[alt]`. Two slots could not express "the gun is Space *and* the left mouse button *and* a shoulder button", so one of the three had to live hardcoded in the game layer, outside both the rebind path and the conflict check. A file written by an older build still loads: `[alt]` is read as `[gamepad]` (#1050)
