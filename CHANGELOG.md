...
- **game**: Observer entity selection and view-from-entity camera — a spectator cycles through live entities with Num1/Num2 and views any of them in Chase or Cockpit (the first pick jumps the free ghost camera into Chase). The picker labels the viewed entity by type name and faction, not a bare index; a since-destroyed pick degrades gracefully back to the free camera. New per-entity `factionIndex` on the snapshot record (client-cached like `typeIndex`, reusable for future friend/foe HUD colouring), a friendly `name` on `MsgEntityTypeDef`, and a `MsgFactionDef` (0x10) index→name table sent once after ConnectAck (#860)
- **game**: Observer ghost-camera flow — joining with `--observer` now brings up a free-fly spectator camera with no ownship instead of stalling the loading screen waiting for an aircraft that never spawns. The client applies the server's planet radius, forces the free camera, seeds the eye above the origin, streams terrain around the camera (not a player position), and skips client-side prediction (no own entity). A mid-session role switch (`set_role`) re-runs the setup: observer→pilot wires prediction and drops into the cockpit; pilot→observer tears prediction down and returns to the ghost camera (#859)
- **network**: Camera-position interest for entity-less peers — an observer (or a dead peer awaiting respawn) has no aircraft transform to key interest management on, so `MsgClientInput` now carries the client's camera eye world-position (`cameraEye`, +24 bytes → 80). The server stores it in `PeerInputState::interestCenter` (finite-guarded) and the per-peer snapshot gather centers `queryRadius` on it when the peer has no entity; a pilot's aircraft transform still wins. Frozen before the parallel peer pass, so it stays serial-equivalent (#858)
