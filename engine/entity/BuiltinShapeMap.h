...
//
// Unknown is the ERROR outcome, not a default: Effect (never spawned today), a Projectile whose
// kind is None (the parser defaults pack projectiles to Missile, so None here is a code bug), and
// any unmapped value all land on the jarring error beacon — a bug must not look like a plausible
// aircraft (#832's lesson).
