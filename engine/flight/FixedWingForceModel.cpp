...
ForceMoment FixedWingForceModel::compute(const FlightState& s, const ControlInput& ctrl, const PayloadEffect& payload,
    ...
    // flameout, or the generic flag) zeroes the propulsive force. A SINGLE engine out removes
    // 1/engine_count of the thrust AND yaws the nose toward the dead engine — the remaining thrust is
    // off the centreline, so it produces a yawing moment proportional to the lost thrust times a
    // moment arm (a fraction of the semi-span, since engine positions are not modelled). This is the
    ...
    if (engineTotalLoss(fail)) {
        ...
        const float arm = data.engine.engine_yaw_arm_frac * data.geometry.wingspan_m;
