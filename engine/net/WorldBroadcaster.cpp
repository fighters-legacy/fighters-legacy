...
WorldBroadcaster::WorldBroadcaster(EntityManager& entityManager, EntityTypeRegistry& registry, INetwork& net,
    ...
    : m_queries(std::move(queries)), m_hooks(std::move(hooks)),
      m_admission(*this, entityManager, registry, net, logger, m_queries, m_hooks), m_entityManager(entityManager),
      m_registry(registry), m_net(net), m_logger(logger), m_weather(weather), m_sensorSystem(entityManager, registry),
